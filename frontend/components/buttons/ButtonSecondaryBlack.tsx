...
const ButtonSecondaryBlack: React.FC<ButtonSecondaryBlackProps> = ({ children, onClick, disabled = false, type = "button", className = "" }) => {
  return (
    <button type={type} onClick={onClick} disabled={disabled} className={`px-4 py-2 border-2 border-black text-black rounded-lg hover:bg-black hover:text-white transition cursor-pointer ${className}`}>
      {children}
    </button>
  );
};
